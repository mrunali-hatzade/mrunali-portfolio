...
- Resume Builder:
  * Section 10 on the page.
  * Allows users to build, edit, and preview a professional tech resume in under 10 minutes.
  * Offers three visual templates: Minimalist ATS (light), Modern Slate (two-column), and Cyberpunk Dev (dark mode matching the site).
  * Generates print-optimized PDF outputs natively through browser printing (window.print() with custom print styling).
...
function getLocalFallbackResponse(text) {
  ...
  if (normalizedText.includes('project') || normalizedText.includes('portfolio') || normalizedText.includes('built') || normalizedText.includes('work') || normalizedText.includes('app')) {
    return "Mrunali has built 15 premium web applications:<br/><br/>" +
           ...
           "  &nbsp;&nbsp;🔗 <a href='https://glamandglow-eight.vercel.app/' target='_blank' rel='noopener noreferrer' class='chat-link'>Live Demo</a> &nbsp;|&nbsp; 💻 <a href='https://github.com/mrunali-hatzade/salon-glam-glow' target='_blank' rel='noopener noreferrer' class='chat-link'>GitHub</a><br/><br/>" +
           "• <strong><a href='#projects' class='chat-link'>JS Salon Akurdi</a></strong> (Salon Website)<br/>" +
  ...
  if (normalizedText.includes('builder') || normalizedText.includes('create resume') || normalizedText.includes('make resume') || normalizedText.includes('cv builder') || normalizedText.includes('resume builder') || normalizedText.includes('build resume') || normalizedText.includes('pdf generator')) {
    return "You can use the <strong><a href='#resume-builder' class='chat-link'>Interactive Resume Builder</a></strong> in Section 10! You can fill out your contact details, work experience, education, skills, and projects, select from three custom templates (ATS Minimal, Modern Slate, and Cyberpunk Dev), and download a print-optimized PDF natively in under 10 minutes!";
  }
